...
def main(args):
    ...
    if args.dataset =='COCO':
        ...
        VOC_dataset_valid = PascalVOC_Dataset(data_dir,
                                          ...
                                          target_transform=encode_labels)
        train_loader = DataLoader(VOC_dataset_train, batch_size=batch_size, num_workers=2, shuffle=True)
        valid_loader = DataLoader(VOC_dataset_valid, batch_size=batch_size, num_workers=2)
    ...
    if args.app == 'train':
        ...
        if args.dataset == 'COCO':
            weights_file_path = '/resnet50/model-1.pth'
            print('pretrained model is resnet50')
        else:
            print('pretrained model is inceptV3')
            weights_file_path = '/inception_v3/model-1.pth'
        ...
        if args.save_results:
            loss, ap, scores, gt = test(args, model, device, valid_loader, returnAllScores=True, num_classes = args.num_classes)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results', default='woSigmoid-BCE-Adam-bs128-box_-1_1_COCO_fixMem', help='path to dataset') 
    ...
    parser.add_argument('--Bs', default=10, type=int,help='the number of class of keeping same in both prediction.')

    parser.add_argument('--myresults', default='./metrics-results/New-CWU-plts.txt', type=str, metavar='M')
    ...
    parser.add_argument('--maxiter', default=100, type=int, help='max iteration to attack')
    ...
    parser.add_argument('--app', default='baseline_FGSM', type=str, \
                        choices={'baseline_CWU', 'baseline_MIFGSMT', 'baseline_DFU', 'baseline_FGSMT', 'baseline_MIFGSM', 'baseline_FGSM', 'baseline_DF', 'baseline_PGD', 'baseline_PGDT',   'baseline_rank', 'baseline_kfool', 'baseline_kUAP', 'test', 'train'}, \
                        help='attack types')
    ...
    for miter in [500]:#
        ...
        for k in [3]:#, 5, 10
            ...
            print('############################')
            print('############################')
            print('now the max iter is :', miter)
            print('now the k value is :', k)
            print('############################')
            print('############################')
            print('############################')
            main(args)
