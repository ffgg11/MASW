...
def test(args, model, device, test_loader, returnAllScores=False, num_classes=20):
    """
    ...
    with torch.no_grad():
        for idx, (data, target) in tqdm(enumerate(test_loader)): 
            if (idx + 1) % 100 == 0:
                print('now idx is :', idx+1)
            #print(data.size(), target.size())
            if not (idx in sample_list):
                continue
            target = target.float()
            if len(target.shape) == 3:
                target = target.max(dim=1)[0]
            ...
            true_sum += (target == 1).long().sum().item()
            # output = model(data.view(-1, c, h, w))
            output = model(data)
            # output = m(output)
            # output = output.view(bs, ncrops, -1).mean(1)
            pred_sum += (output >= 0).long().sum().item()
            loss = criterion(output, target)
            ...
            gc.collect()
            torch.cuda.empty_cache()
    print('pred_sum is :', pred_sum)
    print('true sum is :', true_sum)
